...
function top_magazine_prev_next_posts($order_cat = false) {
	$cmsmasters_post_type = get_post_type();

	$published_posts = wp_count_posts($cmsmasters_post_type)->publish;
	
	
	$prev_post = get_adjacent_post($order_cat, '', true);
	$next_post = get_adjacent_post($order_cat, '', false);
		...
		if($prev_post) {
			echo '<div class="cmsmasters_prev_post">' . 
				'<a alt="' . cmsmasters_title($prev_post->ID, false) . '" title="' . cmsmasters_title($prev_post->ID, false) . '" href="' . get_permalink($prev_post->ID) . '">' . 
					top_magazine_thumb_custom($prev_post->ID, 'cmsmasters-small-thumb', '', false, false) . 
					'<div class="post_nav_cont">' . 
						'<span>' . esc_html__('Prev', 'top-magazine') . '</span>' . 
		...
		if($next_post) {
			echo '<div class="cmsmasters_next_post">' . 
				'<a alt="' . cmsmasters_title($next_post->ID, false) . '" title="' . cmsmasters_title($next_post->ID, false) . '" href="' . get_permalink($next_post->ID) . '">' . 
					top_magazine_thumb_custom($next_post->ID, 'cmsmasters-small-thumb', '', false, false) . 
					'<div class="post_nav_cont">' . 
						'<span>' . esc_html__('Next', 'top-magazine') . '</span>' . 
...
function top_magazine_sharing_box($title_box = false, $tag = 'h3') {
...
function top_magazine_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'top-magazine') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function top_magazine_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
		...
		if ($r_query->have_posts()) {
			echo "<aside class=\"cmsmasters_single_slider\">" . 
				"<" . esc_html($tag) . " class=\"cmsmasters_single_slider_title\">" . 
					($title != '' ? esc_html($title) : esc_html__('More items', 'top-magazine')) . 
				"</" . esc_html($tag) . ">" . 
						...
						while ($r_query->have_posts()) : $r_query->the_post();
							echo "<div class=\"cmsmasters_owl_slider_item cmsmasters_single_slider_item\">
								<div class=\"cmsmasters_single_slider_item_outer\">";
									...
									top_magazine_thumb(get_the_ID(), 'cmsmasters-project-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
...
function top_magazine_author_avatar($template_type = 'page') {
...
function top_magazine_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'top-magazine') . '</' . esc_html($tag) . '>' . "\n" .
