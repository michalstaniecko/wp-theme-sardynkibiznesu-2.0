...
if (
	$cmsmasters_option['top-magazine' . '_profile_post_cat'] || 
	$cmsmasters_option['top-magazine' . '_profile_post_comment'] || 
	(
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_profile'); ?>>
	...
	if ($cmsmasters_profile_title == 'true') {
		echo '<header class="cmsmasters_profile_header entry-header">';
			top_magazine_profile_title_nolink(get_the_ID(), 'h2', $cmsmasters_profile_subtitle, 'h5');
		echo '</header>';
	}
		...
		echo '<div class="profile_sidebar">';
			...
			if ($profile_details == 'true') {
				echo '<div class="profile_details entry-meta">' . 
					...
					top_magazine_get_profile_likes('post');
					
					top_magazine_get_profile_comments('post');
					
					top_magazine_get_profile_features('details', $cmsmasters_profile_features, false, 'h4', true);
					
					top_magazine_get_profile_category(get_the_ID(), 'pl-categs', 'post');
					
				echo '</div>';
			}
			
			
			top_magazine_get_profile_features('features', $cmsmasters_profile_features_one, $cmsmasters_profile_features_one_title, 'h4', true);
			
			top_magazine_get_profile_features('features', $cmsmasters_profile_features_two, $cmsmasters_profile_features_two_title, 'h4', true);
			
			top_magazine_get_profile_features('features', $cmsmasters_profile_features_three, $cmsmasters_profile_features_three_title, 'h4', true);
			
			
			top_magazine_profile_social_icons($cmsmasters_profile_social, get_the_ID(), esc_html__('Socials', 'top-magazine'), 'h4');
		
			if ($cmsmasters_profile_sharing_box == 'true') {
				echo '<div class="profile_details entry-meta">' . 
					'<h4 class="profile_details_title">' . esc_html__('Like this profile?', 'top-magazine') . '</h4>';
...
if ($cmsmasters_option['top-magazine' . '_profile_post_nav_box']) {
	$order_cat = (isset($cmsmasters_option['top-magazine' . '_profile_post_nav_order_cat']) ? $cmsmasters_option['top-magazine' . '_profile_post_nav_order_cat'] : false);
