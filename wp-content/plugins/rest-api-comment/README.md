...
Define SB_S_KEY in wp-config.php file with a secret key to be used in the comment creation.

```php
define('SB_S_KEY', 'your_secret_key');
```

Pass the secret key in the body of the request to create a comment.
...
```Json
{
    ...
    "sb_s_key": "your_secret_key"
}
```

If not defined SB_S_KEY in wp-config.php file, the plugin will allow the creation of comments without the secret key.
