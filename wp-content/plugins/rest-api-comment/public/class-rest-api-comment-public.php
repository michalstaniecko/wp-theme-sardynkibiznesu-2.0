...
if(! class_exists('Rest_Api_Comment_Public')){
	class Rest_Api_Comment_Public {
      ...
      if (defined('SB_S_KEY')) {
        if (empty($request['sb_s_key'])) {
          return new WP_Error('rest_sb_s_key_required',__( 'sb_s_key is required.' ),array( 'status' => 400));
        }

        if ($request['sb_s_key'] !== SB_S_KEY) {
          return new WP_Error('rest_sb_s_key_invalid',__( 'sb_s_key is invalid.' ),array( 'status' => 400));
        }
      }
